...
class Anjuman extends CI_Controller
{
  ...
  public function expense()
  {
    ...
    // On initial load, default to current Financial Hijri year if not specified
  ...
  public function expense_add()
  {
    ...
    // Determine current Financial Hijri year for defaults
    $current_hijri_year = $this->HijriCalendar->get_financial_hijri_year_by_greg_date(date('Y-m-d'));
    ...
    if ($this->input->method() === 'post') {
      ...
      if (!empty($payload['expense_date']) && !empty($payload['amount']) && !empty($payload['payment_mode']) && !empty($payload['hijri_year']) && !empty($payload['item_id'])) {
  ...
  public function expense_edit($id = null)
  {
    ...
    // Current Financial Hijri year for dropdown convenience
    $current_hijri_year = $this->HijriCalendar->get_financial_hijri_year_by_greg_date(date('Y-m-d'));
  ...
  // Qardan Hasana (Anjuman)
  // - /anjuman/qardanhasana
  // - /anjuman/qardanhasana/mohammedi | taher | husain
  // - /anjuman/qardanhasana/{scheme}/import (POST)
  // - /anjuman/qardanhasana/{scheme}/delete/{id} (POST)
  // - /anjuman/qardanhasana/{scheme}/update/{id} (POST)
  public function qardanhasana($scheme = null, $action = null, $id = null)
  {
    ...
    if ($action === 'delete') {
      ...
      redirect('anjuman/qardanhasana/' . $scheme);
      return;
    }
    ...
    // Import CSV disabled for Anjuman
    if ($action === 'import') {
