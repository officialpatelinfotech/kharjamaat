...
<div class="container margintopcontainer">
  <h1 class="text-center pt-5 mb-4"><?php echo $edit_mode ? "Edit Menu" : "Create Menu"; ?></h1>
  <div class="add-item-btn d-flex">
    <a href="<?php echo base_url("Umoor/menulist") ?>" class="btn btn-secondary"><i class="fa-solid fa-arrow-left"></i> Back</a>

    <a href="<?php echo base_url("Umoor/add_menu_item"); ?>" class="btn btn-primary ml-auto">Edit Items</a>
  </div>
  <form method="post" action="<?php echo base_url("Umoor/add_menu") ?>" class="mt-4">
    <div class="form-group">
      <h4><label for="menu_date">Date</label></h4>
      <div class="d-flex flex-column gap-2">
        ...
        <?php endif ?>
        <input type="text" id="menu_date" name="menu_date" class="form-control mb-3" required value="<?php echo isset($menu["date"]) ? date("Y-m-d", strtotime($menu["date"])) : ""; ?>" placeholder="Please select a date">
...
<script>
  $(document).ready(function() {
    ...
    $('#search-input').on('input', function() {
      ...
      $.ajax({
        url: '<?php echo base_url("Umoor/search_items") ?>',
        method: 'POST',
        data: {
          keyword: query
        },
  ...
  `);
    ...
    $("#menu_date").on('change', function(e) {
      $.ajax({
        url: '<?php echo base_url("Umoor/verify_menu_date") ?>',
        method: 'POST',
        data: {
          menu_date: $(this).val()
        },
