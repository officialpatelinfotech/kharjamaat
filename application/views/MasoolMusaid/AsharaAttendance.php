...
<div class="container-fluid margintopcontainer">
  ...
  <div class="card border-0 rounded-0">
    ...
    <div class="card-header text-white py-3 border-0" style="background: linear-gradient(90deg, #d4af37, #b8860b);">
      <div class="d-flex justify-content-between align-items-center">
        ...
        <!-- Right section: Year, Count and button aligned together -->
        <div class="d-flex align-items-center gap-3">
          <?php if (isset($year_options) && is_array($year_options) && !empty($year_options)): ?>
            <div class="d-flex align-items-center gap-2">
              <label for="yearSelect" class="mb-0 small">Hijri Year</label>
              <select id="yearSelect" class="form-select form-select-sm mx-2">
                <?php foreach ($year_options as $y): ?>
                  <option value="<?= (int)$y ?>" <?= (isset($selected_year) && (int)$selected_year === (int)$y) ? 'selected' : '' ?>><?= (int)$y ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          <?php endif; ?>
...
<script>
  // Year switcher: reload with ?year=
  (function() {
...
<script>
  $(function() {
    ...
    $('#saveAttendance').click(function() {
      ...
      $.post('<?php echo base_url("MasoolMusaid/update_attendance") ?>', {
          its,
          day,
          status,
          comment,
    ...
    function populateDayDetailFilters() {
      ...
      $('#filterSector').html('<option value="">All Sectors</option>' + [...sectors].sort().map(s => `<option>${s}</option>`).join(''));
      $('#filterSubSector').html('<option value="">All Sub-Sectors</option>' + [...subs].sort().map(s => `<option>${s}</option>`).join(''));
    }
