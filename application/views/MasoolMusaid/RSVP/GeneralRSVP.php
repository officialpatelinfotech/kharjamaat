...
<div class="container margintopcontainer pt-5">
  ...
  <div class="mb-4 mb-md-0">
    <a href="<?php echo base_url('/MasoolMusaid/rsvp_list') ?>" class="btn btn-outline-secondary inline-block text-blue-600 hover:underline">
      <i class="fas fa-arrow-left"></i>
    </a>
  </div>
  <h3 class="heading text-center mb-4">RSVP <span class="text-primary">Confirmation</span></h3>
  ...
  $total_rsvps = isset($existing_rsvps) ? count($existing_rsvps) : 0;
  ?>
  <div class="row mb-4">
    ...
    <div class="col-12 col-md-6">
      <div class="p-2 text-center shadow-sm border rounded">
        <div class="row">
          <div class="col-6 text-left">
            <span><strong>Total RSVPs Confirmed:</strong></span>
          </div>
          <div class="col-6 text-right">
            <span class="p-2 badge badge-success"><?php echo $total_rsvps; ?></span>
          </div>
        </div>
      </div>
    </div>
  ...
  <form method="post" action="<?php echo base_url('masoolmusaid/submit_general_rsvp'); ?>">
    <input type="hidden" name="miqaat_id" value="<?php echo htmlspecialchars($miqaat_id); ?>">
    <div class="row">
      <div class="col-12">
        <?php if (!empty($members)):
          if (!empty($existing_rsvps)) {
            $rsvp_ids = array_column($existing_rsvps, 'user_id');
          } else {
            $rsvp_ids = [];
          }
        ?>
                ...
                <?php foreach ($members as $member): ?>
                  <tr>
                    <td class="text-center">
                      <input class="m-0 rsvp-checkbox" type="checkbox" name="rsvp_members[]" value="<?php echo htmlspecialchars($member['ITS_ID']); ?>" id="rsvp_<?php echo htmlspecialchars($member['ITS_ID']); ?>" <?php echo in_array($member['ITS_ID'], $rsvp_ids) ? 'checked' : ''; ?>>
                    </td>
    ...
    <div class="submit-footer d-flex justify-content-center align-items-center" style="position:fixed;left:0;right:0;bottom:0;z-index:1050;background:rgba(255,255,255,0.95);box-shadow:0 -2px 8px rgba(0,0,0,0.07);padding:16px 0;">
      <button type="submit" class="btn btn-primary px-5 py-2 fs-5">Submit RSVP</button>
    </div>
  </form>
</div>
<script>
