...
<div class="container margintopcontainer pt-5 mb-5">
            ...
            <div class="col-md-4 col-12 mb-2" id="hofSelectWrapper" style="<?php echo ($member['HOF_FM_TYPE']==='HOF')?'display:none;':''; ?>">
              ...
              <select name="HOF_ID" class="form-control form-select">
                ...
                <?php foreach($hof_list as $h): ?>
                  <option value="<?php echo htmlspecialchars($h['ITS_ID']); ?>" <?php echo ($member['HOF_ID']==$h['ITS_ID'])?'selected':''; ?>><?php echo htmlspecialchars($h['Full_Name']) . ' (' . $h['ITS_ID'] . ')'; ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="col-md-4 col-12 mb-2">
  ...
  <div id="group-marital" class="group-body collapsed border-start border-end border-bottom p-3">
          <div class="row g-3">
            ...
            <div class="col-md-3 col-12">
              ...
              <?php $maritalVal = $member['Marital_Status'] ?? ''; $maritalOptions = ["Single","Married","Engaged","Separated","Divorced","Widowed"]; ?>
            ...
            <div class="col-md-3 col-12">
              ...
              <?php $bloodVal = $member['Blood_Group'] ?? ''; $bloodOptions = ["A+","A-","B+","B-","AB+","AB-","O+","O-","Unknown"]; ?>
  ...
  <div id="group-verification" class="group-body collapsed border-start border-end border-bottom p-3">
          <div class="row g-3">
            ...
            <div class="col-md-4 col-12 mb-2">
              ...
              <?php $inactiveVal = $member['Inactive_Status'] ?? ''; $inactiveOptions=["Deceased","Shifted Jamaat","Travel / Outstation","Duplicate Record","Blocked / Suspended","Other"]; ?>
    ...
    (function(){
      ...
      if(typeSel){
        typeSel.addEventListener('change', function(){
          if(this.value === 'HOF') { hofWrap && (hofWrap.style.display = 'none'); }
          else { hofWrap && (hofWrap.style.display = 'block'); }
        });
      }
      ...
      document.querySelectorAll('.ver-status').forEach(function(sel){
        sel.addEventListener('change', function(){
          ...
          if(!dateInput) return;
          if(this.value === 'Verified'){ if(!dateInput.value) dateInput.value = today(); }
      ...
      function setAll(expand){
        document.querySelectorAll('.group-body').forEach(function(b){
          ...
          if(expand){ b.classList.remove('collapsed'); if(indicator) indicator.textContent='−'; }
          else { b.classList.add('collapsed'); if(indicator) indicator.textContent='+'; }
        });
      }
  ...
  // Leave Inactive_Status empty (controller maps empty to NULL)
        ...
        if(hofType === 'HOF'){ fd.set('HOF_FM_TYPE','HOF'); fd.set('HOF_ID', fd.get('its_id')); }
        else { fd.set('HOF_FM_TYPE','FM'); if(!fd.get('HOF_ID')){ alert('Please select a HOF for this family member'); return; } }
        ...
        fetch(form.action, {method:'POST', body:fd})
          ...
          .then(json=>{
            ...
            if(json.status==='success'){ 
              statusEl.textContent='Saved'; statusEl.className='small text-success';
              setTimeout(()=>{ window.location.href='<?php echo base_url('admin/managemembers'); ?>'; }, 600); 
            } else { 
              statusEl.textContent = json.message || 'Update failed'; statusEl.className='small text-danger';
          ...
          .catch(()=>{ var statusEl=document.getElementById('editMemberStatus'); statusEl.textContent='Network error'; statusEl.className='small text-danger'; });
      });
        ...
        if(sec && sectorMapEdit[sec] && sectorMapEdit[sec].length){
          var frag = document.createDocumentFragment();
            sectorMapEdit[sec].forEach(function(ss){
              var opt=document.createElement('option'); opt.value=ss; opt.textContent=ss; if(ss===preSub) opt.selected=true; frag.appendChild(opt);
            });
      ...
      if(sectorSelectEdit){
        sectorSelectEdit.addEventListener('change', function(){ preSub=''; populateSubEdit(this.value); });
        // initial population
        if(preSector){ populateSubEdit(preSector); }
      }
    })();
  </script>
  <?php else: ?>
