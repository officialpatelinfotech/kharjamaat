...
<div class="container margintopcontainer pt-5">
  <?php if (!empty($qh_import_message)): ?>
    <div class="alert alert-success mb-2 flex-grow-1" style="min-width:260px;"><?php echo htmlspecialchars($qh_import_message); ?></div>
  <?php elseif (!empty($qh_import_error)): ?>
    <div class="alert alert-danger mb-2 flex-grow-1" style="min-width:260px;"><?php echo htmlspecialchars($qh_import_error); ?></div>
  <?php endif; ?>
  ...
  <?php if (isset($scheme_key) && in_array($scheme_key, ['mohammedi', 'taher', 'husain'], true)): ?>
    <?php if ($can_import): ?>
      <div class="d-flex flex-wrap justify-content-end align-items-center gap-2 mb-3">
        <?php if (isset($scheme_key) && $scheme_key === 'mohammedi'): ?>
          <a href="<?php echo base_url($qh_scheme_base . '/mohammedi/template'); ?>" class="btn btn-outline-primary mr-2">
            <i class="fa-solid fa-download"></i> Download CSV Template
          </a>
        <?php endif; ?>
        <?php if (isset($scheme_key) && $scheme_key === 'taher'): ?>
          <a href="<?php echo base_url($qh_scheme_base . '/taher/template'); ?>" class="btn btn-outline-primary mr-2">
            <i class="fa-solid fa-download"></i> Download CSV Template
          </a>
        <?php endif; ?>
        <?php if (isset($scheme_key) && $scheme_key === 'husain'): ?>
          <a href="<?php echo base_url($qh_scheme_base . '/husain/template'); ?>" class="btn btn-outline-primary mr-2">
            <i class="fa-solid fa-download"></i> Download CSV Template
          </a>
        <?php endif; ?>
            ...
            <form method="post" action="<?php echo base_url($qh_scheme_base . '/' . $scheme_key . '/import'); ?>" enctype="multipart/form-data">
              <div class="modal-body">
                <div class="form-group">
                  <label class="mb-2" for="qh-import-file">Upload CSV File</label>
                  ...
                  <div class="form-text text-muted mt-2 small">
                    <div>Only `.csv` files are supported.</div>
                    <?php if (isset($scheme_key) && $scheme_key === 'mohammedi'): ?>
                    ...
                    <?php elseif (isset($scheme_key) && $scheme_key === 'taher'): ?>
      ...
      <div class="card shadow-sm mb-4 qh-filters-card">
        <div class="card-body p-2">
          <form method="get" action="<?php echo base_url($qh_scheme_base . '/' . $scheme_key); ?>" class="form-row d-flex flex-wrap align-items-end">
            <div class="form-group col-2 mr-2 mb-2" style="min-width:220px;">
            ...
            <?php if (isset($scheme_key) && in_array($scheme_key, ['taher', 'husain'], true)): ?>
              ...
              ?>
              <div class="form-group col-auto mr-2 mb-2" style="min-width:260px;">
                <label for="search">Member Name / ITS</label>
                <input type="text" class="form-control" id="search" name="search" placeholder="Search by ITS or Member Name" value="<?php echo htmlspecialchars($combinedSearch); ?>" />
              </div>
            <?php endif; ?>
    ...
    <?php if ($can_manage): ?>
      ...
      <script>
        (function() {
          ...
          var fileLabel = document.querySelector('label[for="qh-import-file"].custom-file-label');
          var update = function() {
