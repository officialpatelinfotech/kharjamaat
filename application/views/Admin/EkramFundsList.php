<div class="container margintopcontainer pt-5">
  ...
  <h4 class="heading text-center mb-4">Created Ekram Funds</h4>
  <div class="card shadow-sm">
    <div class="card-body p-0">
      <table class="table table-hover m-0" id="ekramFundsTable">
        <thead class="thead-light">
          <tr>
            <th data-sort-key="sr" class="sortable">Sr. No</th>
            <th data-sort-key="title" class="sortable">Year</th>
        ...
        <tbody>
          <?php if (isset($funds) && !empty($funds)): $sr = 1; foreach ($funds as $f): ?>
              <tr data-fund-row="<?php echo (int)$f['id']; ?>">
                <td><?php echo $sr++; ?></td>
                <td class="fund-title" data-fund-title><?php echo isset($f['hijri_year']) && $f['hijri_year'] ? htmlspecialchars($f['hijri_year']) : preg_replace('/[^0-9]/','',htmlspecialchars($f['title'])); ?></td>
            ...
            <tr>
              <td colspan="5" class="text-center text-muted">No ekram funds created yet.</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
...
<div class="modal" tabindex="-1" role="dialog" id="editFundModal" style="display:none; background:rgba(0,0,0,0.5); position:fixed; top:0; left:0; right:0; bottom:0;">
  <div class="modal-dialog" role="document" style="max-width:500px; margin:60px auto;">
      ...
      <div class="modal-header">
        <h5 class="modal-title">Edit Ekram Fund</h5>
...
<script>
  (function() {
        ...
        function sortTableBy(key, direction) {
          var table = document.getElementById('ekramFundsTable');
          if (!table) return;
        ...
        [].forEach.call(document.querySelectorAll('#ekramFundsTable thead th.sortable'), function(th) {
    ...
    saveBtn.addEventListener('click', function() {
      ...
      $.ajax({
        url: '<?php echo site_url('admin/ekramfunds_update_fund'); ?>',
        method: 'POST',
        data: payload,
        processData: false,
        contentType: false,
    ...
    [].forEach.call(document.querySelectorAll('.delete-fund-btn'), function(btn) {
      btn.addEventListener('click', function() {
        ...
        $.ajax({
          url: '<?php echo site_url('admin/ekramfunds_delete_fund'); ?>',
          method: 'POST',
          data: payload,
          processData: false,
          contentType: false,
