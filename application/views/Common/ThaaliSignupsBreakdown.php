...
?>
<style>
  .sticky-header thead th { position: sticky; top: 0; z-index: 2; background: #fff; }
  .progress-xxs { height: 6px; }
  .card.hoverable:hover { transform: translateY(-1px); box-shadow: 0 .5rem 1rem rgba(0,0,0,.08); transition: all .15s ease; }
  ...
  @keyframes rowPulse {
    0% { background-color: #fffceb; }
    40% { background-color: #fff5c2; }
...
<div class="container margintopcontainer pt-4">
  ...
  <form method="get" action="<?php echo site_url('common/thaali_signups_breakdown'); ?>" class="row g-2 align-items-end mb-3" id="tsbForm">
    <?php if (!empty($from)): ?>
    ...
    <div class="col-auto">
      <?php 
        ...
        if (!empty($from)) { $clearHref .= '?from=' . urlencode($from); }
      ?>
  ...
  <style>
    .hijri-switcher .chev-pill {
      ...
      padding:8px 12px;border-radius:999px;border:1px solid #e6eef0;background:#fff;color:#0b5563;cursor:pointer;transition:all .12s ease;box-shadow:none;
      min-width:44px;min-height:44px;
    }
    .hijri-switcher .chev-pill i { font-size:1rem; }
    .hijri-switcher .chev-pill:hover { transform:translateY(-2px); box-shadow:0 8px 20px rgba(15,23,42,0.06); }
    .hijri-switcher .hijri-title { color:#0ea5a4;font-weight:700;font-size:1.05rem;padding:0 8px; }
    .hijri-switcher .hijri-nav-btn.disabled .chev-pill { opacity:.45; pointer-events:none; }
    @media (max-width:576px) {
      .hijri-switcher .chev-pill { padding:6px 8px; min-width:38px; min-height:38px; }
      .hijri-switcher .hijri-title { font-size:0.95rem; }
    }
  </style>
