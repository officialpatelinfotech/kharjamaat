...
// Build sector/sub-sector lists from assignments
$sectorList = [];
$subSectorList = [];
      ...
      <div class="form-row align-items-end">
        ...
        <div class="col-md-3">
          ...
          <select id="filterFund" class="form-control form-control-sm">
            ...
            <?php if (!empty($ekram_details)) {
              foreach ($ekram_details as $group) {
                $fund = $group['fund'];
                $label = !empty($fund['hijri_year']) ? $fund['hijri_year'] : ($fund['title'] ?? ($fund['name'] ?? 'Fund')); ?>
                <option value="<?= (int)($fund['id'] ?? 0); ?>"><?= htmlspecialchars($label); ?></option>
            <?php }
            } ?>
          </select>
        </div>
      ...
      foreach ($ekram_details as $group) {
        ...
        <?php foreach (($group['rows'] ?? []) as $r) { ?>
          allRows.push({
            ...
            fund_name: <?= json_encode(!empty($fund['hijri_year']) ? $fund['hijri_year'] : ($fund['title'] ?? ($fund['name'] ?? ''))); ?>,
