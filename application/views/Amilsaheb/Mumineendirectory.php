...
.excl-trigger{display:flex;align-items:center;justify-content:space-between;width:100%;height:32px;padding:0 10px;border:1.5px solid rgba(185,28,28,.4);border-radius:7px;background:#fff8f8;font-family:'Plus Jakarta Sans',sans-serif;font-size:.76rem;color:var(--text-1);cursor:pointer;transition:border-color .15s;user-select:none}
...
function readURLAndApply(){
  ...
  const isDash=!!((legFilter&&legFilter!=='all')||statusP||itsMatchP||madresaP||minP||maxP||maritalP);
  ...
  if(isDash){
    ...
    const dashMap={'status':'fStatus','activity_status':'fStatus','health_status':'fHealth','deeni_status':'fDeeni','residential_status':'fResidential','gender':'fGender','hof_fm_type':'fHOFType','its_sabeel_match':'fItsMatch','marital_status':'fMarital','marital':'fMarital','ms':'fMarital'};
...
function run(){
  ...
  // Collect exclude selections from checkboxes
  ...
  filtered=ALL_DATA.filter(u=>{
    ...
    // Exclude filters — remove member if their status is in the exclude list
  ...
  row.querySelectorAll('.chip-x').forEach(x=>{
    x.addEventListener('click',()=>{
      if(x.dataset.exclVal){
        ...
        if(cb){cb.checked=false;updateExclTrigger();run();}
      } else {
        const el=document.getElementById(x.dataset.id);if(el){el.value='';run();}
      }
    });
  });
}
...
function resetAll(){
  ...
  ['secLabel2','secRow2','secLabel3','secRow3','secLabel4','secRow4','maritalCol'].forEach(id=>{const e=document.getElementById(id);if(e)e.style.display=''});
...
// Exclude checkboxes: fire run() on any change
...
(()=>{
  ...
  // Move panel to body so parent overflow:hidden cannot clip it
