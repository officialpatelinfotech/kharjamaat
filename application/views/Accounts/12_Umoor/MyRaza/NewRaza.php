...
<script>
  ...
  function submit_raza(current) {
    ...
    // Before submitting, fetch member dues and show confirmation modal
    fetch('<?php echo base_url('accounts/get_member_dues') ?>', {
      ...
      .then(function(resp) {
        if (!resp || !resp.success) {
          // proceed directly if unable to fetch dues
          doRazaSubmit(data);
          return;
        }
        var d = resp.dues;
        ...
        showDuesModal(html);
        // bind proceed button
        var proceedBtn = document.getElementById('dues-confirm');
        proceedBtn.onclick = function() {
          // optionally notify then submit
          fetch('<?php echo base_url('accounts/send_dues_email') ?>', {
              method: 'POST',
              credentials: 'same-origin'
            })
            .finally(function() {
