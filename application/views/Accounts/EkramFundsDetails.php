...
<div class="container margintopcontainer pt-5 pb-5">
  ...
  <div class="page-header-wrap pt-5">
    ...
    <h1 class="page-heading">Ekram Funds</h1>
  </div>
  <p class="page-sub">Ekram fund assignments, payments &amp; outstanding dues</p>
  ...
  <div class="section-card">
    <div class="section-card-header">
      <h5 class="section-card-title"><i class="fa fa-list"></i> Per Year Breakdown</h5>
    </div>
    <div class="table-responsive">
      <table class="themed-table">
        ...
        <tbody>
          <?php
          $rows = $ekram_details['rows'] ?? [];
          if (!empty($rows)):
            $i = 1;
            foreach ($rows as $r): ?>
            <tr>
              <td><span class="t-num"><?php echo $i++; ?></span></td>
              <td><span class="t-title"><?php
                $year = '';
                if (!empty($r['hijri_year'])) {
                  $year = (string)$r['hijri_year'];
                } else {
                  $title = $r['title'] ?? '';
                  if (preg_match('/(\d{3,4})/', $title, $m)) { $year = $m[1]; }
                  else { $year = $title; }
                }
